...
class WordSentRegressorWithStats(nn.Module):
    """
        Word Sentence Regression model.
        Args:
            word_embed_dim: Dimension of the word embeddings
            sent_embed_dim: Dimension of the sentence embeddings
    """
    def __init__(self, word_embed_dim:int, sent_embed_dim:int, num_stats_features:int ,*args, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.word_lstm_1 = nn.LSTM(word_embed_dim, 128, batch_first=True)
        self.word_lstm_2 = nn.LSTM(128, 64, batch_first=True)
        self.sent_lstm_1 = nn.LSTM(sent_embed_dim, 128, batch_first=True)
        self.sent_lstm_2 = nn.LSTM(128, 64, batch_first=True)
        self.stats_seq = nn.Sequential(
            nn.Linear(num_stats_features, 8),
            nn.ReLU(),
            nn.Linear(8, 2),
            nn.ReLU()
        )
        # Since adding stats features, increasing the input dimension for fc1
        self.fc1 = nn.Linear(130, 64)
        self.fc2 = nn.Linear(64, 1)
        self.relu = nn.ReLU()

    def forward(self, word_embeds, sent_embeds, stats):
        word_lstm_out, _ = self.word_lstm_1(word_embeds)
        word_lstm_out = self.relu(word_lstm_out)
        word_lstm_out, _ = self.word_lstm_2(word_lstm_out)
        word_lstm_out = self.relu(word_lstm_out)
        ...
        stats_out = self.stats_seq(stats)
        
        
        combined = torch.cat([word_lstm_out, sent_lstm_out,stats_out], dim=1)
